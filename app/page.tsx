...
export default function Home() {
  ...
  const nextSession = sessions.find((item) => item.date > today && item.category !== "Descanso");
  const currentWeekKey = todaySession?.week ?? sessions.find((item) => item.date >= today)?.week ?? sessions.at(-1)?.week;
  const currentWeek = sessions.filter((item) => item.week === currentWeekKey);
  ...
  const filteredSessions = useMemo(() => {
    ...
    return sessions.filter((item) => (category === "Todos" || item.category === category) && (!query || `${item.session} ${item.notes} ${item.weekTitle}`.toLocaleLowerCase("es").includes(query)));
  }, [search, category]);
  ...
  return <div className="app-shell">
    ...
    {selected && <RecordModal session={selected} record={records[selected.id]} onClose={() => setSelected(null)} onSubmit={saveRecord} />}
  </div>;
}
